...
public class IFI_Flow extends PlugInFrame implements ActionListener, ItemListener, MouseListener {
    ...
    public void mouseReleased(MouseEvent m) {
        ...
        int x2 = m.getX();
        int y2 = m.getY();
        ox = canvas.offScreenX(x2);
        oy = canvas.offScreenY(y2);
        if (checkCent.getState()) {
            Center2D();
        }
    ...
    void Dots() {
        ...
        for (i = 0; i < (rt.getCounter()); i++) {
            ...
            if (checkText.getState()) {
                ...
                ip2.setFont(font);
                ip2.drawString("" + nbtrack, cx + (dotsize - 5) / 2, cy - (dotsize - 5) / 2);
            }
            nbtrackold = nbtrack;
        }
