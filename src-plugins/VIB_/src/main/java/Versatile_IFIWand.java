 ...
 *
 * Usage ===== - When called from the plugins menu, the menu provides an option
 * to install the tool in the ImageJ Toolbar. This replaces all other custom
 * tools, e.g. those from the startup macros. - Put the following into your
 * ImageJ/macros/StartupMacros.txt file to have the Versatile Wand as a standard
 * tool:
 *
 ...
 * can be selected.
 *
 * Compilation =========== Note that there is a line "gd.addHelp..." in
 * showDialog that cannot be compiled with ImageJ versions before 1.42p. Remove
 * that line if you are running an old version of ImageJ.
 *
 * Version 1.13, Michael Schmid, 2012-Dec-07: Bugfix RGB tolerance was incorrect
 * Version 1.12, Michael Schmid, 2010-Sep-10: Can use eyedropper (foreground)
 * color instead of the color (value) of the pixel Version 1.11, Michael Schmid,
 * 2009-Jul-10: Color, non-contiguous added. NullPointerException fixed.
 *
 */
public class Versatile_IFIWand  {
    ...
    // dialog parameters
    static double valueTolerance = 240;
    static double gradientTolerance = 0;
    static double colorSensitivity = -100;
    static int connection = EIGHT_CONNECTED;
    static boolean includeHoles = true;
    static boolean useEyedropper = true;
    static int xStart, yStart;
    boolean colorMode = true;
    ImagePlus imp;

    public void doWand(ImagePlus imp, int x0, int y0) {
        ...
        grayRef = calTable == null ? index : calTable[index];    // reference color from this pixel
...
//            roi.update(shiftKeyDown, altKeyDown);
    ...
    boolean checkColor(int rgb, int r0, int g0, int b0, double rWeight, double gWeight, double bWeight) {
        ...
        double deltaSqr = rWeight * deltaR * deltaR + gWeight * deltaG * deltaG + bWeight * deltaB * deltaB;
